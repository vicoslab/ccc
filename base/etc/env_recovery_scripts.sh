...
# /etc/environment so sshd/PAM sessions get them even when the user's shell does
# not read /etc/profile.d, e.g. `ssh container command`, non-bash shells, and PTY
# sessions that bypass normal shell startup files.
init_pam_environment_vars() {
